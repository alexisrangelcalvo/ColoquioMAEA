...
#net_differences_bymonths$QUANTILE_AGE = as.factor(net_differences_bymonths$QUANTILE_AGE)
...
```{r}
gglimbp2_2 =  ggplot(net_differences_bymonths) +
  geom_boxplot(mapping = aes(x=SEX, y = LIMIT_BAL, fill= CLIENT_PROFILE))+ theme(plot.title=element_text(size=15))+
  ggtitle("Distribución del limite crediticio por sexo y perfil del cliente")
...
```{r}
gglimbp3_2 = ggplot(net_differences_bymonths) +
  geom_boxplot(mapping = aes(x=EDUCATION, y = LIMIT_BAL, fill=CLIENT_PROFILE))+ theme(plot.title=element_text(size=14))+
  ggtitle("Distribución del limite crediticio por educación y perfil del cliente")
gglimbp3_2
```
```{r}
gglimbp4_2 = ggplot(net_differences_bymonths) +
  geom_boxplot(mapping = aes(x=MARRIAGE, y = LIMIT_BAL, fill=CLIENT_PROFILE))+ theme(plot.title=element_text(size=14))+
  ggtitle("Distribución del limite crediticio por civil y perfil del cliente")
gglimbp4_2
```


```{r}
unique(net_differences_bymonths$QUANTILE_AGE)
#levels(net_differences_bymonths$QUANTILE_AGE) <- c('42 - 79 anios', '35 - 41 anios', '29 - 34 anios','21 - 28 anios')
...
gglimbp5_2 = ggplot(net_differences_bymonths) +
  geom_boxplot(mapping = aes(x=QUANTILE_AGE, y = LIMIT_BAL, fill=CLIENT_PROFILE))+ theme(plot.title=element_text(size=14))+
  ggtitle("Distribución del limite crediticio por quantil de edad y perfil")
gglimbp5_2
```
...
net_differences_bymonths2 = select(net_differences_bymonths, CLIENT_PROFILE, EDUCATION, MARRIAGE, SEX, LIMIT_BAL)
...
propGlob
propGlob_f = propGlob %>% as.data.frame.matrix() %>% mutate(variables = c("graduate school", "university", "high school", "others", "TOTALES")) %>% select(variables, Perfil_1, Perfil_2, Perfil_3, Perfil_4,Perfil_5) %>% flextable(col_keys =  c("variables", "Perfil_1","Perfil_2", "Perfil_3", "Perfil_4", "Perfil_5")) %>% theme_vader()
propGlob_f
...
propGlob
propGlob_f = propGlob %>% as.data.frame.matrix() %>% mutate(variables = c("Casado", "Soltero", "Divorciado", "TOTALES")) %>% select(variables, Perfil_1, Perfil_2, Perfil_3, Perfil_4,Perfil_5) %>% flextable(col_keys =  c("variables", "Perfil_1","Perfil_2", "Perfil_3", "Perfil_4", "Perfil_5")) %>% theme_vader()
propGlob_f
```
...
propGlob
propGlob_f = propGlob %>% as.data.frame.matrix() %>% mutate(variables = c("Hombre", "Mujer", "TOTALES")) %>% select(variables, Perfil_1, Perfil_2, Perfil_3, Perfil_4,Perfil_5) %>% flextable(col_keys =  c("variables", "Perfil_1","Perfil_2", "Perfil_3", "Perfil_4", "Perfil_5")) %>% theme_vader()
propGlob_f
...
###################### REALIZANDO OTRA VEZ LOS ANOVAS Y PAREOS ##################
...
anova1 = aov(bd_anovas$LIMIT_BAL ~ bd_anovas$MARRIAGE)
summary(anova1)
TukeyHSD(anova1)
res1 = residuals(anova1)
lillie.test(res1)

anova2 = aov(bd_anovas$LIMIT_BAL ~ bd_anovas$EDUCATION)
summary(anova2)
TukeyHSD(anova2)
#res2 = residuals(anova2)
#lillie.test(res2)

anova3 = aov(bd_anovas$LIMIT_BAL ~ bd_anovas$SEX)
summary(anova3)
TukeyHSD(anova3)
#res3 = residuals(anova3)
#lillie.test(res3)

anova4 = aov(bd_anovas$LIMIT_BAL ~ bd_anovas$QUANTILE_AGE)
summary(anova4)
TukeyHSD(anova4)
#res4 = residuals(anova4)
#lillie.test(res4)

anova5 = aov(bd_anovas$LIMIT_BAL ~ bd_anovas$CLIENT_PROFILE)
summary(anova5)
TukeyHSD(anova5)
#res5 = residuals(anova5)
#lillie.test(res5)
...
```{r}
#NORMALIDAD
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$MARRIAGE == "single"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$MARRIAGE == "married"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$MARRIAGE == "divorced"] ) 

lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$EDUCATION == "university"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$EDUCATION == "graduate school"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$EDUCATION == "high school"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$EDUCATION == "others"] ) 

lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$CLIENT_PROFILE == "Perfil_1"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$CLIENT_PROFILE == "Perfil_2"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$CLIENT_PROFILE == "Perfil_3"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$CLIENT_PROFILE == "Perfil_4"] ) 
lillie.test( bd_anovas$LIMIT_BAL[bd_anovas$CLIENT_PROFILE == "Perfil_5"] ) 

#No se rechaza la hipotesis nula, no  hay noramlidad en nuestros datos

#HOMOGENEIDAD
bartlett.test( LIMIT_BAL ~ MARRIAGE, 
               data = bd_anovas )
bartlett.test( LIMIT_BAL ~ EDUCATION, 
               data = bd_anovas )
bartlett.test( LIMIT_BAL ~ CLIENT_PROFILE, 
               data = bd_anovas )
#No se rechaza la hipotesis nula, no  hay homogeneidad en las varianzas
```
...
pairwise.t.test( bd_anovas$LIMIT_BAL, 
                 ...
                 p.adj = "bonferroni")
#
kruskal.test(LIMIT_BAL ~ CLIENT_PROFILE, data = bd_anovas)

pairwise.t.test( bd_anovas$LIMIT_BAL, 
                 bd_anovas$CLIENT_PROFILE, 
                 p.adj = "bonferroni")
