...
```{r}
ggplot(df0) +
  geom_boxplot(mapping = aes(x=EDUCATION, y = AGE, fill=SEX))
```
...
```{r}
ggplot(df0) +
  geom_boxplot(mapping = aes(x=EDUCATION, y = LIMIT_BAL, fill=SEX))
```


¿Por qué parece que se tienen más registros de hombres que de mujeres en las ultimas trazas?
```{r}
ggplot(df0) +
...
net_differences_bymonths$CLIENT_PROFILE = as.factor(net_differences_bymonths$CLIENT_PROFILE)
...
```{r}
ggplot(net_differences_bymonths) +
...
unique(net_differences_bymonths$EDUCATION)
ggplot(net_differences_bymonths, aes( x= CLIENT_PROFILE, y = EDUCATION, fill = LIMIT_BAL)) +
  geom_tile(color = "white",
            lwd = 0.1,
            linetype = 1)+
  guides(fill = guide_colourbar(barwidth = 0.5,
                                barheight = 12))+ theme(plot.title=element_text(size=10))+
  ggtitle("Limite crediticio por perfil y nivel educativo")
```
...
Sacando correlaciones entre:

- El perfil del cliente dado por la moda del estatus de pago a lo largo de los 6 meses y el limit bal.
- Educación y el limit bal
- Sexo y el limit bal
- Estado civil y el limit bal

-Limit bal y nivel educativo (generar grafica también)
-Gasto promedio mensual y nivel educativo (generar grafica también)
- GPM y sexo (generar grafica también)
- GPM y perfil del cliente (generar grafica también)
